      ...
      containers: [
        {
          name: 'api'
          image: appImage
          // The image's default CMD omits --proxy-headers; set it here so the
          // real client IP (behind the ACA ingress) reaches the app for the
          // rate limiter + sign-in log. * is safe: only the ACA ingress fronts it.
          command: [
            'uvicorn'
            'app.main:app'
            '--host'
...
resource workerApp 'Microsoft.App/containerApps@2024-03-01' = {
  ...
  properties: {
    ...
    template: {
      ...
      scale: {
        // min 1 so a leader always exists; scales up on pending-job depth.
        minReplicas: 1
