...
async def mta_sts_builder(
    ...
    wildcard is actively worse than none (the exact bug found on
    davids.online this session: mx: *.mx.microsoft looked plausible but
    didn't actually cover the real two-label MX host)."""
    domain = await _get_owned_domain(db, domain_id, user.organization_id)
