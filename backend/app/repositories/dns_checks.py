...
def _apply_tls_rpt_filters(
    ...
    `until` is exclusive (`date_range_begin < until`), same semantics as
    dmarc_reports.py's _apply_report_filters — the router computes it as
    "the day after date_to, at 00:00:00 UTC" so the whole of date_to is
    included."""
    if since is not None:
        query = query.where(TlsRptReport.date_range_begin >= since)
    if until is not None:
        query = query.where(TlsRptReport.date_range_begin < until)
...
async def list_tls_rpt_reports_for_domain(
    ...
    *,
    ...
    query = _apply_tls_rpt_filters(query, since=since, until=until, org_name=org_name, failures_only=failures_only)
