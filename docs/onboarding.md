...
scoped. Written from the multi-tenant model the app actually has (an
"operator" who can create/manage organizations, and the organizations
themselves) — that's a description of the app's own architecture, not a
claim that this is run as a commercial service for paying clients.

## 1. Platform admin: create the organization

In the admin console (`/admin`), enter the organization's name and its
**Entra tenant ID** (the GUID for its Microsoft 365 / Azure AD tenant —
found in its Entra admin center under "Tenant properties"). Providing the
tenant ID up front activates the organization immediately.
...
from the same screen — nothing else is blocked on it except that
organization's users being able to sign in.

That's the only manual step on the platform-admin side. Everything from here
is self-service from the organization's own dashboard.

## 2. Organization: sign in

The organization's users go to the dashboard and sign in with "Sign in with
...
the organization's tenant is configured to block user consent entirely, in
which case an admin-consent link for the SSO app is available from the
mailbox setup screen (see below) and its Global Admin needs to click it once.

The first user to sign in for a newly-activated organization automatically
becomes that organization's `org_admin`.

## 3. Organization (org_admin): connect the mailbox

On first visit to the dashboard, a banner prompts for the shared mailbox
address that receives DMARC/TLS-RPT reports (e.g.
`dmarc-reports@yourdomain.com`). Entering it and clicking "Save & start
...
more things need to happen on the organization's side, both surfaced as
links right there in the banner (click "Entra links" once the banner is up,
or — if the first sync fails — the same links appear automatically under
the sync-error message):

1. **Mail access consent** (required): the organization's Global Admin
   clicks the mail-access admin-consent link and approves it. This grants
   the app's Mail Access application (app-only, `Mail.Read`) permission in
   its tenant.
2. **Exchange Application Access Policy** (required, done outside this app):
   ...
   ```powershell
   New-DistributionGroup -Name "DMARC Dashboard Mailbox Access" -Members dmarc-reports@yourdomain.com
   ...
   Test-ApplicationAccessPolicy -AppId <mail-access-app-id> -Identity dmarc-reports@yourdomain.com
   ```
...
If the organization sets the mailbox address before completing these two
steps (a common ordering, since it's the more discoverable action), the
first sync will fail. That's expected — the dashboard shows the same
consent-guidance links directly next to the sync error, with no dead end.
...
## 5. Organization (org_admin): add domains
...
## 6. Organization (org_admin): add DKIM selectors

DKIM selectors aren't discoverable via DNS alone, so they have to be added
manually (whatever selector the mail provider uses — check its provider's
DKIM setup docs, e.g. `selector1`/`selector2` for Microsoft 365, `google`
for Google Workspace). Without at least one selector added, the DKIM check
just reports "no selectors registered" rather than a real result.
...
"Recheck". **This app is read-only and advisory toward the organization's
DNS — it never writes records on their behalf.** Every finding is a
recommendation; the organization updates its own authoritative DNS (at its
